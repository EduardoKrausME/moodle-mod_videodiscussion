...
const mergeSegments = segments => {
    const clean = segments
        .filter(segment => Array.isArray(segment) && segment.length >= 2 && segment[1] > segment[0])
        .map(segment => [Number(segment[0]), Number(segment[1])])
        .sort((a, b) => a[0] - b[0]);
    const merged = [];
    clean.forEach(segment => {
        if (!merged.length || segment[0] > merged[merged.length - 1][1] + 0.75) {
            merged.push(segment);
        } else {
...
export const init = async () => {
    const root = document.querySelector('[data-region="videodiscussion"]');
    if (!root) {
        return;
    }
    let config = {};
    ...
    root.querySelectorAll('[data-vd-seek], [data-vd-marker]').forEach(element => {
        element.addEventListener('click', async event => {
            event.preventDefault();
            await adapter.seek(Number(element.dataset.time || 0));
    ...
    const commentButton = root.querySelector('[data-action="comment-now"]');
    if (commentButton) {
        commentButton.addEventListener('click', async () => {
            const time = await adapter.currentTime();
            const form = root.querySelector('[data-region="new-discussion-form"]');
            const timeInput = root.querySelector('[data-region="new-timepoint"]');
            const timeLabel = root.querySelector('[data-region="new-timecode"]');
    ...
    let segments = Array.isArray(config.segments) ? config.segments : [];
    let lastSample = null;
    let dirty = false;
    let saving = false;
    ...
    const saveProgress = async () => {
        ...
        try {
            ...
            const label = root.querySelector('[data-region="progress-label"]');
            const bar = root.querySelector('[data-region="progress-bar"]');
            if (label) {
                label.textContent = `${Number(result.percent).toFixed(2)}%`;
            }
            if (bar) {
                bar.style.width = `${Math.min(100, Math.max(0, Number(result.percent)))}%`;
            }
            dirty = false;
        } catch (error) {
            Notification.exception(error);
        ...
        try {
            ...
            if (playing && lastSample !== null) {
                ...
                if (delta > 0.1 && delta <= 2.5) {
                    segments.push([lastSample, position]);
